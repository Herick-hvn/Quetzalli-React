...
@Component({
  selector: 'app-agregar-recetas',
  templateUrl: './agregar-recetas.component.html',
  styleUrls: ['./agregar-recetas.component.scss']
})
export class AgregarRecetasComponent {
  ...
  agregar() {
    if (this.formularioValido() && this.costoProduccionTotal > 0) {
      ...
      this.recetasService.addRecetaAndMaterials(nuevaReceta, pulseraMateriales).subscribe(
        (recetaAgregada: any) => {
          if (recetaAgregada) {
            console.log('Receta y materiales agregados con éxito:', recetaAgregada);
